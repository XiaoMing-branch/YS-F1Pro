...
typedef struct
{
  vu16 LCD_REG;	   //偏移量0x00
  vu16 LCD_RAM;	   //偏移量0x02   因为是2个字节
} LCD_TypeDef;

/* Note: LCD /CS is CE4 - Bank 4 of NOR/SRAM Bank 1~4 */
#define LCD_BASE        ((u32)(0x60000000 | 0x0C000000))
#define LCD             ((LCD_TypeDef *)LCD_BASE)  //将0x6c000000强制转换为LCD_TypeDef类型的指针

//-------------------------------------------------------------------------------
//以上定义也可按如下定义，与上面等同
//#define  LCD_REG    0x06c000000
//#define  LCD_RAM    0x06c000002

//下面赋值的时候改成  *(vu16 *)LCD_REG = 0x????;	 *(vu16 *)LCD_RAM = 0x????;
//在硬件中，当FSMC的A0为低电平时，数值写入REG;当FSMC的A0为高电平时，数值写入RAM;
//由于数据宽度为16位，占两个字节，所以REG的地址为0x06c000000，RAM的地址为0x06c000002
//如果数据宽度为8位，则REG的地址为0x06c000000，RAM的地址为0x06c000001
